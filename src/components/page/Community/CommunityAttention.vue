...
<script setup>
import { ref } from 'vue';
// 移除了仅在动态列表中使用的 Pointer, Star, Warning 图标
import { ArrowRight, Search, Bell } from '@element-plus/icons-vue';
import 'element-plus/theme-chalk/display.css';
import HomePageHead from '@/components/homepage/Head.vue';
import DynamicList from '@/components/community/DynamicList.vue'; // <-- 请确保路径正确

const newPostText = ref('');

// 模拟的动态数据仍然保留在这里，以便传递给子组件
// 将来这里可以替换为从 API 获取数据的逻辑
const posts = ref([
    { 
    id: 1, 
...
@media (max-width: 767px) {
  ...
  /* 移除了针对 .post-content 和 .image-gallery 的响应式样式 */
}
